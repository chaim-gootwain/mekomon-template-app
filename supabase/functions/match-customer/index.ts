...
/* ---------- ניקוד דמיון (ללא ספריות חיצוניות) ---------- */ function norm(s) {
  return String(s || '').replace(/["'`״׳]/g, '').replace(/[-–—ـ]/g, ' ').replace(/[^֐-׿a-zA-Z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
}
/* אותיות סופיות → רגילות, כדי ש"עתון"/"עיתון" ו-ם/מ לא יפריעו */ function foldFinals(s) {
  return s.replace(/ם/g, 'מ').replace(/ן/g, 'נ').replace(/ץ/g, 'צ').replace(/ף/g, 'פ').replace(/ך/g, 'כ');
}
function words(s) {
  return foldFinals(norm(s)).split(' ').filter(Boolean);
}
...
/* דמיון בין שני שמות: ממוצע ההתאמות הטובות ביותר מילה-מול-מילה,
                                                                                   אדיש לסדר המילים; מילה בלי בת-זוג מורידה את הציון. */ function nameSim(qWords, cWords) {
...
/* תחיליות יחס נפוצות בתחילת השם מהמלל ("לגן ורדים" → גם "גן ורדים") */ function queryVariants(raw) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!name) return json({
      error: 'לא התקבל שם לקוח'
    }, 400);
    const { data: customers, error } = await svc.from('customers').select('id, name, invoice_name, business_id, phone, email');
    if (error) return json({
      error: 'שגיאה בטעינת לקוחות: ' + error.message
    }, 500);
    const scored = (customers || []).map((c)=>({
        ...
        score: Math.round(scoreCustomer(name, c) * 100) / 100
      })).filter((c)=>c.score >= 0.55).sort((a, b)=>b.score - a.score).slice(0, 6);

...
// הצבת {placeholder} מתוך ה-payload בתבנית ההודעה של הכלל
...
function moneyIL(v) {
  const n = Number(v);
  return Number.isFinite(n) ? '₪' + n.toLocaleString('he-IL', { maximumFractionDigits: 2 }) : String(v ?? '');
}
...
function buildAlertText(eventType, rule, payload) {
  ...
  if (eventType === 'debt_over_threshold') {
    return {
      ...
      body: rule.template
        ? fillTemplate(rule.template, vars)
        : `ללקוח ${payload?.customer_name || ''} חוב פתוח של ${vars.balance} (מעל הסף ${vars.threshold})`,
      severity: 'warning',
    };
  }
  return {
...
async function stepProcess(svc) {
  ...
  if (!events?.length) return { events_processed: 0, alerts_created: 0, suppressed: 0 };

  const types = [...new Set(events.map(e => e.event_type))];
  const { data: rules, error: rErr } = await svc.from('alert_rules')
    .select('*').eq('enabled', true).in('event_type', types);
  ...
  for (const ev of events) {
    ...
    for (const rule of matching) {
      ...
      const { data: existing, error: dErr } = await svc.from('alerts')
        .select('id,status,created_at').eq('dedup_key', dedupKey)
        .or(`status.eq.new,created_at.gte.${since}`).limit(1);
      if (dErr) throw new Error('process/dedup: ' + dErr.message);
      if (existing?.length) { suppressed++; continue; }
      ...
      const { data: alertRow, error: aErr } = await svc.from('alerts').insert({
        ...
        title: txt.title, body: txt.body, severity: txt.severity,
        dedup_key: dedupKey, status: 'new',
      }).select('id').single();
      if (aErr) {
        // 23505 = הפרת האינדקס הייחודי החלקי (מרוץ) — נחשב כבלימה, לא ככשל
        if (String(aErr.code) === '23505') { suppressed++; continue; }
        throw new Error('process/insert: ' + aErr.message);
      }

      const channels = (rule.channels?.length ? rule.channels : ['inapp'])
        .filter(c => ['inapp', 'email', 'whatsapp'].includes(c));
      const rows = channels.map(c => ({ alert_id: alertRow.id, channel: c, status: 'queued' }));
      if (rows.length) {
    ...
    const { error: mErr } = await svc.from('alert_events')
      .update({ processed_at: new Date().toISOString() }).eq('id', ev.id);
    if (mErr) throw new Error('process/mark: ' + mErr.message);
  }
  return { events_processed: events.length, alerts_created: created, suppressed };
}

/* ----- ערוצי המשלוח ----- */

// inapp: ההתראה כבר קיימת בטבלת alerts (זה הערוץ) — רק מסמנים שנשלח
...
async function stepDeliver(svc, settings) {
  const { data: queued, error: qErr } = await svc.from('alert_deliveries')
    .select('*').eq('status', 'queued')
    .order('created_at', { ascending: true }).limit(BATCH);
  if (qErr) throw new Error('deliver/queued: ' + qErr.message);
  if (!queued?.length) return { delivered: 0, failed: 0 };
  ...
  for (const d of queued) {
    ...
    if (d.channel === 'inapp') {
      ...
      res = settings.alerts_email_enabled === '1'
        ? await sendEmailStub() : { ok: false, error: 'channel not configured' };
    } else if (d.channel === 'whatsapp') {
      res = settings.alerts_whatsapp_enabled === '1'
        ? await sendWhatsappStub() : { ok: false, error: 'channel not configured' };
    } else {
    ...
    const patch = res.ok
      ...
      : { status: 'failed', error: res.error || 'send failed' };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ----- מתג ראשי -----
    const { data: sRows } = await svc.from('settings').select('key,value')
      .in('key', ['alerts_enabled', 'alerts_email_enabled', 'alerts_whatsapp_enabled']);
    const settings = {};
    (sRows || []).forEach(r => settings[r.key] = r.value);
    if (settings.alerts_enabled !== '1') {
      return json({ ok: true, skipped: 'alerts_disabled' });
    }

    let action = 'run';
    try { action = (await req.json())?.action || 'run'; } catch (_) { /* גוף ריק → run */ }
    ...
    if (!['run', 'scan', 'process', 'deliver'].includes(action)) {

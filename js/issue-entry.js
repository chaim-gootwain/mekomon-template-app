...
- בדיקת חשבונית של הלקוח (מתוך החיוביים)
...
${canWrite ? `<td style="white-space:nowrap"><button class="btn btn-sm btn-ghost" onclick="ieCustEdit(${a.customer_id})">✎ לקוח</button> <button class="btn btn-sm btn-ghost" onclick="ieDealEdit(${a.customer_id})">✎ עסקה</button> <button class="btn btn-sm btn-danger-ghost" onclick="ieDelAd(${a.id})">🗑</button></td>` : ''}
</tr>`;
...
============================================================ */
/* עריכת פרטי הלקוח ישירות מרשימת הגיליון — טוען את הרשומה המלאה,
שומר, ומרענן את הרשימה במקום (בלי לזרוק למסך הלקוחות) */
async function ieCustEdit(custId) {
let c;
try { c = await run(db.from('customers').select('*').eq('id', custId).single()); } catch (e) { }
if (!c) { toast('לקוח לא נמצא', true); return; }
openForm('עריכת לקוח — ' + c.name, CUSTOMER_FIELDS, c, async (rec) => {
await run(db.from('customers').update(rec).eq('id', custId));
try { await addInteraction('customer', custId, '✏️ עריכת פרטי לקוח (מהזנת גיליון ' + (_ieIssue ? _ieIssue.issue_number : '') + ')'); } catch (e) { }
await refreshCache();

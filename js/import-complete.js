...
------------------------------------------------------------
- לקוחות שיובאו מסומנים בהערה "יובא-מגיליון-<מס'> ... להשלמת פרטים"
  (מספר הגיליון נלקח מההערה עצמה — לא מקודד בתבנית)
...
const IMPORT_MARK = 'יובא-מגיליון-';
const IMPORT_MARK_RX = /יובא-מגיליון-(\d+)/;
let _icQueue = [];
let _icIdx = 0;
...
function _icCleanNotes(notes, size) {
  const m = String(notes || '').match(IMPORT_MARK_RX);
  const keep = String(notes || '').split('\n').filter(ln => !ln.includes(IMPORT_MARK)).join('\n').trim();
  const tag = 'מפרסם' + (m ? ' גיליון ' + m[1] : ' (יובא)') + (size ? ' · ' + size : '');
  return keep ? tag + '\n' + keep : tag;
}

...
<div class="grid2">
<div class="field"><label>מספר פרסומים בחבילה *</label><input id="subInserts" type="number" value="${c.total_inserts != null ? c.total_inserts : 13}" dir="ltr" oninput="subPriceRecalc()"></div>
<div class="field"><label>אופן תמחור</label><select id="subPriceMode" onchange="subPriceRecalc()"><option value="total">מחיר כולל לחבילה</option><option value="per">מחיר לפרסום</option></select></div>
</div>
<div class="field"><label id="subPriceLbl">מחיר כולל (₪, לפני מע"מ) *</label><input id="subPrice" type="number" value="${c.total_price != null ? c.total_price : ''}" dir="ltr" oninput="subPriceRecalc()">
<p id="subPriceHint" class="muted" style="font-size:.76rem;margin-top:2px"></p></div>
...
try { subPriceRecalc(); } catch (e) { }
// אתחול שורות התאריכים
if (cad === 'selected') { if (dates.length) dates.forEach(d => subAddDate(d)); else subAddDate(); }
}

function subPriceRecalc() {
const modeEl = document.getElementById('subPriceMode'); if (!modeEl) return;
const mode = modeEl.value;
const price = Number(document.getElementById('subPrice').value) || 0;
const n = Math.max(1, Number(document.getElementById('subInserts').value) || 1);
const lbl = document.getElementById('subPriceLbl'), hint = document.getElementById('subPriceHint');
if (mode === 'per') {
if (lbl) lbl.textContent = 'מחיר לפרסום (₪, לפני מע"מ) *';
if (hint) hint.textContent = 'סה"כ לחבילה: ' + (money(Math.round(price * n)) || '₪0');
} else {
if (lbl) lbl.textContent = 'מחיר כולל לחבילה (₪, לפני מע"מ) *';
if (hint) hint.textContent = 'מחיר לפרסום: ' + (money(Math.round(price / n)) || '₪0');
}
}
...
const _pmode = document.getElementById('subPriceMode') ? document.getElementById('subPriceMode').value : 'total';
const _pval = Number(document.getElementById('subPrice').value) || 0;
const _ptotal = _pmode === 'per' ? Math.round(_pval * Math.max(1, inserts)) : Math.round(_pval);
const rec = {
...
if (!['admin', 'editor'].includes(profile.role)) return false; // מנהל + אחראי עימוד
try {
const { issue, items } = await subDueForUpcoming();
if (!issue || !items.length) return false;
if (document.getElementById('subRemOverlay')) return true;
const _vb = document.getElementById('viewBack');
if (_vb && _vb.classList.contains('open')) return false; // חלון אחר פתוח (למשל אשף ייבוא) — לא דורסים
emuEnsureStyles();
